var calendar = {
	...
	drawCld: function(SY, SM) {
		...
		if(Array.from(calendar.cld).filter(item => item.isToday).length === 0){
			const index = newsDate1.findIndex(item => item.text === 30);
			if(index > -1){
				newsDate1[index].isToday = true;
			}else{
				if(this.isleapYear(calendar.cld[0].sYear)){
					const index = newsDate1.findIndex(item => item.text === 29);
					newsDate1[index].isToday = true;
				}else{
					const index = newsDate1.findIndex(item => item.text === 28);
					newsDate1[index].isToday = true;
				}
			}
		}
	...
	mOvr: function(v) {
		...
		if (calendar.cld[d] == undefined) {
			if(calendar.cld[0].sMonth === 2){
				if(this.isleapYear(calendar.cld[0].sYear)){ // 是闰年
					d = 28;
				}else{
					d = 27;
				}
		...
		let _om = calendar.cld[d].sMonth < 10 ? ('0' + calendar.cld[d].sMonth) : calendar.cld[d].sMonth;
		let _od = calendar.cld[d].sDay < 10 ? ('0' + calendar.cld[d].sDay) : calendar.cld[d].sDay;
		oDate = calendar.cld[d].sYear + '-' + _om + '-' + _od;
		oMonth = calendar.cld[d].sYear + '-' + _om;
		oWeek = '星期' + calendar.cld[d].week;
		if (calendar.show == 0) {
		...
		calendar.oDate = oDate;
		calendar.oMonth = oMonth;
		calendar.oWeek = oWeek;
	},
